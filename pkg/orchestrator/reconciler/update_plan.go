...
func (v *instanceView) serving() bool {
	return v.Available && !v.Terminating && v.Class != instancectl.CompatBroken
}
...
func newInstanceView(inst *types.Instance, class instancectl.CompatClass, minReady time.Duration, now time.Time) instanceView {
...
func planUpdate(in updateInput) updatePlan {
	...
	for i := range in.Instances {
		...
		if v.Class == instancectl.CompatOutdated {
	...
	for i := range in.Instances {
		v := &in.Instances[i]
		if !retired[v.Instance.ID] && !v.Terminating && v.Class == instancectl.CompatOK {
			currentTemplate++
		}
	}
