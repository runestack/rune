...
func view(name string, class instancectl.CompatClass, available bool, ageSeconds int) instanceView {
	inst := &types.Instance{
		ID:        name,
		Name:      name,
...
func TestPlanUpdate_WorkedExample(t *testing.T) {
	...
	insts := []instanceView{
		view("A", instancectl.CompatOutdated, true, 40),
		view("B", instancectl.CompatOutdated, true, 30),
		view("C", instancectl.CompatOutdated, true, 20),
		view("D", instancectl.CompatOutdated, true, 10),
	}
	...
	insts = append(insts, view("E", instancectl.CompatOK, false, 0))
	p = planUpdate(updateInput{Scale: 4, Params: surgeable(), Instances: insts, Now: planBase})
	...
	insts[4] = view("E", instancectl.CompatOK, true, 0)
	p = planUpdate(updateInput{Scale: 4, Params: surgeable(), Instances: insts, Now: planBase})
	...
	insts = []instanceView{
		...
		view("C", instancectl.CompatOutdated, true, 20),
		view("D", instancectl.CompatOutdated, true, 10),
		view("E", instancectl.CompatOK, true, 0),
	}
	p = planUpdate(updateInput{Scale: 4, Params: surgeable(), Instances: insts, Now: planBase})
...
func TestPlanUpdate_ConvergedIsQuiet(t *testing.T) {
	insts := []instanceView{
		view("a", instancectl.CompatOK, true, 30),
		view("b", instancectl.CompatOK, true, 20),
	}
...
func TestPlanUpdate_NotAvailableOutdatedRetiresOutsideBudget(t *testing.T) {
	insts := []instanceView{
		view("old", instancectl.CompatOutdated, true, 60),
		view("stuck", instancectl.CompatOutdated, false, 30), // never became ready
	}
...
func TestPlanUpdate_FloorHoldsWhileRetiringUnavailable(t *testing.T) {
	insts := []instanceView{
		view("s1", instancectl.CompatOutdated, false, 40),
		view("s2", instancectl.CompatOutdated, false, 30),
		view("ok", instancectl.CompatOutdated, true, 20),
	}
...
func TestPlanUpdate_RepairIsUnbudgeted(t *testing.T) {
	insts := []instanceView{
		view("broken", instancectl.CompatBroken, false, 40),
		view("a", instancectl.CompatOK, true, 30),
		view("b", instancectl.CompatOK, true, 20),
	}
	p := planUpdate(updateInput{Scale: 3, Params: surgeable(), Instances: insts, Now: planBase})
...
func TestPlanUpdate_RepairProceedsDuringUpdate(t *testing.T) {
	insts := []instanceView{
		view("old1", instancectl.CompatOutdated, true, 50),
		view("old2", instancectl.CompatOutdated, true, 40),
		view("broken", instancectl.CompatBroken, false, 30),
		view("new", instancectl.CompatOK, false, 10), // surge slot taken, not ready
	}
	p := planUpdate(updateInput{Scale: 3, Params: surgeable(), Instances: insts, Now: planBase})
...
func TestPlanUpdate_RepairAndCreateDoNotOverProvision(t *testing.T) {
	insts := []instanceView{
		view("broken", instancectl.CompatBroken, false, 40),
		view("ok", instancectl.CompatOK, true, 30),
	}
	p := planUpdate(updateInput{Scale: 2, Params: surgeable(), Instances: insts, Now: planBase})
	...
	settled := []instanceView{
		view("ok", instancectl.CompatOK, true, 30),
		view("repaired", instancectl.CompatOK, true, 0),
	}
...
func TestPlanUpdate_RecreateTakesEverythingDownFirst(t *testing.T) {
	...
	insts := []instanceView{
		view("a", instancectl.CompatOutdated, true, 30),
		view("b", instancectl.CompatOutdated, true, 20),
		view("c", instancectl.CompatOutdated, true, 10),
	}
...
func TestPlanUpdate_ScaleDownDoesNotFightTheSurge(t *testing.T) {
	insts := []instanceView{
		...
		view("C", instancectl.CompatOutdated, true, 20),
		view("D", instancectl.CompatOutdated, true, 10),
		view("E", instancectl.CompatOK, false, 0), // the surged replacement, still starting
	}
	p := planUpdate(updateInput{Scale: 4, Params: surgeable(), Instances: insts, Now: planBase})
...
func TestPlanUpdate_GenuineScaleDownRetiresOldest(t *testing.T) {
	insts := []instanceView{
		view("oldest", instancectl.CompatOK, true, 40),
		view("middle", instancectl.CompatOK, true, 30),
		view("newest", instancectl.CompatOK, true, 10),
	}
...
func TestPlanUpdate_ScaleDownPrefersOutdatedThenBroken(t *testing.T) {
	insts := []instanceView{
		view("current", instancectl.CompatOK, true, 40),
		view("outdated", instancectl.CompatOutdated, true, 30),
		view("broken", instancectl.CompatBroken, false, 20),
	}
	p := planUpdate(updateInput{Scale: 1, Params: surgeable(), Instances: insts, Now: planBase})
...
func TestPlanUpdate_ScaleUpCreatesDeficit(t *testing.T) {
	insts := []instanceView{view("a", instancectl.CompatOK, true, 30)}
	p := planUpdate(updateInput{Scale: 4, Params: surgeable(), Instances: insts, Now: planBase})
...
func TestPlanUpdate_ExclusiveServiceRetiresBeforeCreating(t *testing.T) {
	...
	insts = []instanceView{view("b", instancectl.CompatOutdated, true, 20)}
	p = planUpdate(updateInput{Scale: 2, Params: exclusive(), Instances: insts, Now: planBase})
...
func TestNewInstanceView_MinReadyGate(t *testing.T) {
	...
	v := newInstanceView(fresh, instancectl.CompatOK, 5*time.Second, planBase)
	assert.True(t, v.Ready)
	assert.False(t, v.Available, "Running for 1s with a 5s window is not yet serving")

	v = newInstanceView(settled, instancectl.CompatOK, 5*time.Second, planBase)
	...
	v = newInstanceView(legacy, instancectl.CompatOK, 5*time.Second, planBase)
...
func TestPlanUpdate_Property_NeverBreachesFloorOrCap(t *testing.T) {
	...
	classes := []instancectl.CompatClass{instancectl.CompatOK, instancectl.CompatOutdated, instancectl.CompatBroken}
	...
	for trial := 0; trial < 3000; trial++ {
		...
		working := func(v *instanceView) bool {
			return v.Ready && !v.Terminating && v.Class != instancectl.CompatBroken
		}
...
func snapshotCounts(vs []instanceView) planCounts {
	...
	for i := range vs {
		...
		if v.Class == instancectl.CompatOutdated && !v.Terminating {
			c.outdated++
		}
	}
	return c
...
func dumpViews(vs []instanceView) string {
	...
	for _, v := range vs {
		cls := "OK"
		switch v.Class {
		case instancectl.CompatOutdated:
			cls = "OUTDATED"
		case instancectl.CompatBroken:
			cls = "BROKEN"
		}
...
func TestPlanUpdate_Property_AlwaysConverges(t *testing.T) {
	...
	for trial := 0; trial < 500; trial++ {
		...
		for i := 0; i < scale; i++ {
			insts = append(insts, view(fmt.Sprintf("old%d", i), instancectl.CompatOutdated, true, 100-i))
		}
		...
		for tick := 0; tick < maxTicks; tick++ {
			...
			for i := range remaining {
				if remaining[i].Class == instancectl.CompatOK {
					remaining[i].Available = true
					remaining[i].Ready = true
				}
			}
			for i := 0; i < p.Create+len(p.Repair); i++ {
				v := view(fmt.Sprintf("new%d", next), instancectl.CompatOK, false, 0)
			...
			outdated := 0
			for i := range insts {
				if insts[i].Class == instancectl.CompatOutdated {
					outdated++
				}
			}
