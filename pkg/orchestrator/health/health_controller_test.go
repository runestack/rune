...
func setupHealthController(t *testing.T) (context.Context, *store.TestStore, *runner.TestRunner, Controller) {
	...
	instanceController := instancectl.NewController(testStore, testRunnerMgr, testLogger)

	controller := NewController(testLogger, testStore, testRunnerMgr, instanceController)
