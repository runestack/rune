...
func setupTestServiceController(t *testing.T) (context.Context, *store.TestStore, *instancectl.FakeController, *health.FakeController, Controller) {
	ctx := context.Background()
	testStore := store.NewTestStore()
	testInstanceController := instancectl.NewFakeController()
	mockHealthController := health.NewFakeController()
...
func TestServiceEvent_EnqueuesKey(t *testing.T) {
	...
	assert.Equal(t, 1, sc.rec.QueueLen(), "created event must enqueue")
	...
	assert.Equal(t, 1, sc.rec.QueueLen(), "same key must coalesce, not duplicate")
	...
	assert.Equal(t, 2, sc.rec.QueueLen(), "deleted event must enqueue its own key")
}
...
func TestStatusOnlyUpdate_SkipsEnqueue(t *testing.T) {
	...
	assert.Equal(t, 0, sc.rec.QueueLen(), "status-only echo (pointer) must not enqueue")
	...
	assert.Equal(t, 0, sc.rec.QueueLen(), "status-only echo (value) must not enqueue")
	...
	assert.Equal(t, 1, sc.rec.QueueLen(), "uninspectable update must enqueue")
}
...
func TestSyncService_NotFoundIsTerminal(t *testing.T) {
	...
	err := sc.rec.SyncService(ctx, "default/ghost")
...
func TestSyncService_MalformedKeyDropped(t *testing.T) {
	...
	err := sc.rec.SyncService(ctx, "no-separator")
...
func TestResyncEnqueuesAll(t *testing.T) {
	...
	require.NoError(t, sc.rec.EnqueueAll(ctx))
	assert.Equal(t, 3, sc.rec.QueueLen(), "every service must be enqueued on resync")
}
...
func TestInstanceEvent_EnqueuesOwner(t *testing.T) {
	...
	assert.Equal(t, 1, sc.rec.QueueLen(), "instance event must enqueue its owner")
	...
	assert.Equal(t, 1, sc.rec.QueueLen(), "same owner must coalesce")
	...
	assert.Equal(t, 1, sc.rec.QueueLen(), "ownerless instance must not enqueue")
}
...
func TestInstanceEvent_ReconcilerSourceSuppressed(t *testing.T) {
	...
	assert.Equal(t, 0, sc.rec.QueueLen(), "reconciler-sourced instance events are echoes and must be dropped")
}
...
func TestInstanceEvent_FallsBackToStoreLookup(t *testing.T) {
	...
	assert.Equal(t, 1, sc.rec.QueueLen(), "store lookup must resolve the owner")
	...
	assert.Equal(t, 1, sc.rec.QueueLen(), "unmappable deleted instance must be dropped")
}
...
func TestInstanceChange_RollsUpStatusEventDriven(t *testing.T) {
	ctx, testStore, fakeInstanceController, _, controller := setupTestServiceController(t)
	sc := controller.(*serviceController)
	sc.rec.SetReconcileInterval(60 * time.Second) // resync can't help us
...
func TestGetServiceLogs_NoCapturedOutput_SynthesizesWhyLine(t *testing.T) {
	...
	fakeInstanceController.GetLogsFunc = func(ctx context.Context, instance *types.Instance, _ types.LogOptions) (io.ReadCloser, error) {
		return io.NopCloser(strings.NewReader(instancectl.SynthesizeNoLogsLine(instance))), nil
	}
