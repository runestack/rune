...
// Controller implements the Controller interface for service management
...
// serviceController implements the Controller interface
...
func NewController(
	...
	// reconciler's slice to reconciler.New — the embedded pair, not the
	// old god interface.
	instanceController interface {
		serviceInstanceOps
		reconciler.InstanceOps
	},
	healthController health.Controller,
	logger log.Logger,
) (Controller, error) {
...
func (sc *serviceController) enqueueServiceEvent(event store.WatchEvent) {
	...
	sc.rec.Enqueue(event.Namespace, event.Name)
}
...
func (sc *serviceController) GetServiceLogs(ctx context.Context, namespace, name string, opts types.LogOptions) (io.ReadCloser, error) {
	...
	peekers := make([]*instancectl.PeekingReader, 0)
	for _, instance := range instances {
		if instance.Status != types.InstanceStatusRunning {
			continue
		}
		...
		pr := instancectl.NewPeekingReader(logReader)
...
func (sc *serviceController) enqueueInstanceEvent(event store.WatchEvent) {
	...
	sc.rec.Enqueue(namespace, serviceName)
}
...
func (sc *serviceController) handleRealDeletion(ctx context.Context, service *types.Service, request *types.DeletionRequest, finalizerTypes []types.FinalizerType) (*types.DeletionResponse, error) {
	...
	sc.rec.Enqueue(service.Namespace, service.Name)
...
var _ serviceInstanceOps = (*instancectl.Controller)(nil)
