...
func setupDrainFixture(t *testing.T, n int, drain time.Duration) (context.Context, *store.TestStore, *runner.TestRunner, *Controller, *seqLog, []*types.Instance) {
	t.Helper()
...
func TestRestartInstance_WithdrawsBeforeStop(t *testing.T) {
	...
	require.NoError(t, c.RestartInstance(ctx, inst, RestartReasonManual))
