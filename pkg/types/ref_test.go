...
func TestParseResourceRefWithDefaults(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "ns/name shorthand crosses namespaces under typed default",
			in:          "platform/ghcr-credentials",
			defaultType: ResourceTypeSecret,
			defaultNS:   "default",
			want:        ResourceRef{Type: ResourceTypeSecret, Namespace: "platform", Name: "ghcr-credentials"},
		},
		{
			name:        "ns/name shorthand works without a typed default too",
			in:          "platform/ghcr-credentials",
			defaultType: "",
			defaultNS:   "default",
			want:        ResourceRef{Type: "", Namespace: "platform", Name: "ghcr-credentials"},
		},
		{
			name:        "ns/name shorthand with empty namespace is rejected",
			in:          "/ghcr-credentials",
			defaultType: ResourceTypeSecret,
			defaultNS:   "default",
			wantErr:     true,
		...
		{
			name:        "ns/name shorthand with empty name is rejected",
			in:          "platform/",
			defaultType: ResourceTypeSecret,
			defaultNS:   "default",
			wantErr:     true,
		...
		{
			name:        "bare name with two slashes is rejected",
			in:          "platform/ghcr/credentials",
			defaultType: ResourceTypeSecret,
			defaultNS:   "default",
			wantErr:     true,
