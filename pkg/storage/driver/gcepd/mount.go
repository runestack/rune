...
func (execMounter) EnsureFormatted(ctx context.Context, dev, fsType string) error {
	if fsType == "" {
		return errors.New("gcepd: EnsureFormatted called with empty fsType")
	}
	...
	if _, err := exec.LookPath(mkfs); err != nil {
		return fmt.Errorf("gcepd: %s not in PATH: %w", mkfs, err)
	}
	cmd := exec.CommandContext(ctx, mkfs, dev)
	if combined, err := cmd.CombinedOutput(); err != nil {
		return fmt.Errorf("gcepd: %s %s: %w (%s)", mkfs, dev, err, strings.TrimSpace(string(combined)))
	}
	return nil
}
